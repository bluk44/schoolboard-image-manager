...
public class EditionFrame extends javax.swing.JFrame {
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				EditionFrame inst = new EditionFrame();
	...
	public EditionFrame(BufferedImage image){
		super();
		initImageComp(image);
		initGUI();
		addEventListeners();
		...
		addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				if(e.getKeyCode() == KeyEvent.VK_ENTER && src.isBoardSelected()){
					BufferedImage extracted = ImageProcessing.
							extractBoardRegion(src.getImage(), src.getDistorted(), src.getFixed());
					src.unsetSelectionMode();
					Test.showImage(extracted, "exteracted board region");
					src.repaint();
