...
public class Labeling4 {
	...
	public static List<Region> run(BufferedImage binaryImage){
		...
		for(int i = 0; i < data.getSizeY(); i++){
			for(int j = 0; j < data.getSizeX(); j++){
				...
				if(w == 0 && n == 0){
					...
					labelIndex.put(l, idx);
					++l;
					//System.out.println();
					//System.out.println(labelSets.getSets());
				} else if(w != 0 && n != 0){
					data.setElement(j, i, w);
					labelSets.union(labelIndex.get(w), labelIndex.get(n));
				} else {
		...
		for(int i = 0; i < data.getSizeY(); i++){
			for(int j = 0; j < data.getSizeX(); j++){
				if(data.getElement(j, i) == BG) continue;
				int rootIdx = labelSets.find(labelIndex.get(data.getElement(j, i)));
